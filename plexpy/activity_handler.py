...
class ActivityHandler(object):
    ...
    def on_start(self):
        if self.is_valid_session():
            ...
            logger.debug(
                u"Tautulli ActivityHandler :: Session %s started by user %s (%s) with ratingKey %s (%s)."
                % (str(session['session_key']), str(
                    session['user_id']), session['username'],
                   str(session['rating_key']), session['full_title']))

            plexpy.NOTIFY_QUEUE.put({
                'stream_data': session.copy(),
            ...
            schedule_callback('session_key-{}'.format(self.get_session_key()),
                              ...
                              args=[
                                  ...
                                  session['full_title'], session['username']
                              ],
                              minutes=5)
    ...
    def on_stop(self, force_stop=False):
        if self.is_valid_session():
            logger.debug(
                u"Tautulli ActivityHandler :: Session %s %sstopped." %
            ...
            if row_id:
                ...
                logger.debug(
                    u"Tautulli ActivityHandler :: Removing sessionKey %s ratingKey %s from session queue"
                    %
    ...
    def on_pause(self, still_paused=False):
        if self.is_valid_session():
            if not still_paused:
                logger.debug(
                    u"Tautulli ActivityHandler :: Session %s paused." %
            ...
            if not still_paused:
                plexpy.NOTIFY_QUEUE.put({
                    'stream_data': db_session.copy(),
                    'notify_action': 'on_pause'
                })

    def on_resume(self):
        if self.is_valid_session():
            logger.debug(u"Tautulli ActivityHandler :: Session %s resumed." %
                         str(self.get_session_key()))
    ...
    def on_change(self):
        if self.is_valid_session():
            logger.debug(
                u"Tautulli ActivityHandler :: Session %s has changed transcode decision."
                % str(self.get_session_key()))
            ...
            plexpy.NOTIFY_QUEUE.put({
                'stream_data': db_session.copy(),
                'notify_action': 'on_change'
            })

    def on_buffer(self):
        if self.is_valid_session():
            logger.debug(
                u"Tautulli ActivityHandler :: Session %s is buffering." %
                self.get_session_key())
            ap = activity_processor.ActivityProcessor()
            db_stream = ap.get_session_by_key(
                session_key=self.get_session_key())

            # Increment our buffer count
            ap.increment_session_buffer_count(
                session_key=self.get_session_key())

            # Get our current buffer count
            current_buffer_count = ap.get_session_buffer_count(
                self.get_session_key())
            logger.debug(
                u"Tautulli ActivityHandler :: Session %s buffer count is %s." %
                (self.get_session_key(), current_buffer_count))

            # Get our last triggered time
            buffer_last_triggered = ap.get_session_buffer_trigger_time(
                self.get_session_key())
            ...
            if buffer_last_triggered:
                logger.debug(
                    u"Tautulli ActivityHandler :: Session %s buffer last triggered at %s."
                    % (self.get_session_key(), buffer_last_triggered))
                time_since_last_trigger = int(
                    time.time()) - int(buffer_last_triggered)
            ...
            if current_buffer_count >= plexpy.CONFIG.BUFFER_THRESHOLD and time_since_last_trigger is None or \
                ...
                ap.set_session_buffer_trigger_time(
                    session_key=self.get_session_key())
    ...
    if ACTIVITY_SCHED.get_job(id):
        if remove_job:
            ...
            ACTIVITY_SCHED.reschedule_job(
                id,
                args=args,
                trigger=DateTrigger(run_date=datetime.datetime.now(pytz.UTC) +
...
def force_stop_stream(session_key, title, user):
    ...
    if row_id:
        ...
        logger.info(
            u"Tautulli ActivityHandler :: Removing stale stream with sessionKey %s ratingKey %s from session queue"
...
def clear_recently_added_queue(rating_key, title):
    ...
    if plexpy.CONFIG.NOTIFY_GROUP_RECENTLY_ADDED_GRANDPARENT and len(
            child_keys) > 1:
        ...
        for child_key in child_keys:
            ...
            if plexpy.CONFIG.NOTIFY_GROUP_RECENTLY_ADDED_PARENT and len(
                    grandchild_keys) > 1:
...
def on_created(rating_key, **kwargs):
    logger.debug(
        u"Tautulli TimelineHandler :: Library item %s added to Plex." %
        str(rating_key))
    pms_connect = pmsconnect.PmsConnect()
    ...
    if metadata:
        ...
        if 'child_keys' not in kwargs:
            if data_factory.get_recently_added_item(rating_key):
                logger.debug(
                    u"Tautulli TimelineHandler :: Library item %s added already. Not notifying again."
                    % str(rating_key))
                notify = False
        ...
        logger.debug(u"Added %s items to the recently_added database table." %
                     str(len(all_keys)))

    else:
        logger.error(
            u"Tautulli TimelineHandler :: Unable to retrieve metadata for rating_key %s"
            % str(rating_key))
    ...
    try:
        os.remove(
            os.path.join(
                ...
                'session_metadata/metadata-sessionKey-%s.json' % session_key))
    except OSError as e:
        logger.error(
            u"Tautulli ActivityHandler :: Failed to remove metadata cache file (sessionKey %s): %s"
            % (session_key, e))
