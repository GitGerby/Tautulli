...
def refresh_libraries():
    ...
    if not server_id:
        logger.error(
            u"Tautulli Libraries :: No PMS identifier, cannot refresh libraries. Verify server in settings."
        )
        return
    ...
    if library_sections:
        ...
        for section in library_sections:
            ...
            result = monitor_db.upsert('library_sections',
                                       key_dict=section_keys,
                                       value_dict=section_values)
...
def update_section_ids():
    ...
    try:
        ...
        logger.warn(
            u"Tautulli Libraries :: Unable to execute database query for update_section_ids: %s."
            % e)

        logger.warn(
            u"Tautulli Libraries :: Unable to update section_id's in database."
        )
        plexpy.CONFIG.UPDATE_SECTION_IDS = 1
        plexpy.CONFIG.write()
        return None
    ...
    for library in library_results:
        ...
        if section_type != 'photo':
            ...
            if library_children:
                ...
                key_mappings.update({
                    child['rating_key']: child['section_id']
                    for child in children_list
                })
            else:
                logger.warn(
                    u"Tautulli Libraries :: Unable to get a list of library items for section_id %s."
                    % section_id)
    ...
    for item in history_results:
        ...
        if section_id:
            try:
                ...
                monitor_db.upsert('session_history_metadata',
                                  key_dict=section_keys,
                                  value_dict=section_values)
            except:
    ...
    if error_keys:
        logger.info(
            u"Tautulli Libraries :: Updated all section_id's in database except for rating_keys: %s."
            % ', '.join(str(key) for key in error_keys))
    else:
        logger.info(
            u"Tautulli Libraries :: Updated all section_id's in database.")
...
def update_labels():
    ...
    try:
        ...
        logger.warn(
            u"Tautulli Libraries :: Unable to execute database query for update_labels: %s."
            % e)

        logger.warn(
            u"Tautulli Libraries :: Unable to update labels in database.")
        plexpy.CONFIG.UPDATE_LABELS = 1
        plexpy.CONFIG.write()
        return None
    ...
    for library in library_results:
        ...
        if section_type != 'photo':
            library_children = []
            library_labels = pms_connect.get_library_label_details(
            ...
            if library_labels:
                for label in library_labels:
                    ...
                    if library_children:
                        ...
                        for rating_key in [
                                child['rating_key'] for child in children_list
                        ]:
                            if key_mappings.get(rating_key):
                                key_mappings[rating_key].append(
                                    label['label_title'])
                            else:
                                key_mappings[rating_key] = [
                                    label['label_title']
                                ]
    ...
    for rating_key, labels in key_mappings.iteritems():
        try:
            labels = ';'.join(labels)
            monitor_db.action(
                'UPDATE session_history_metadata SET labels = ? '
    ...
    if error_keys:
        logger.info(
            u"Tautulli Libraries :: Updated all labels in database except for rating_keys: %s."
            % ', '.join(str(key) for key in error_keys))
    else:
...
class Libraries(object):
    ...
    def get_datatables_list(self, kwargs=None):
        ...
        if session.get_session_shared_libraries():
            custom_where.append([
                ...
                session.get_session_shared_libraries()
            ])
        ...
        columns = [
            ...
            'session_history_metadata.labels', 'library_sections.do_notify',
        ...
        try:
            ...
            logger.warn(
                u"Tautulli Libraries :: Unable to execute database query for get_list: %s."
                % e)
    ...
    def get_datatables_media_info(self,
        ...
        if section_id and not str(section_id).isdigit():
            logger.warn(
                u"Tautulli Libraries :: Datatable media info called but invalid section_id provided."
            )
            return default_return
        elif rating_key and not str(rating_key).isdigit():
            logger.warn(
                u"Tautulli Libraries :: Datatable media info called but invalid rating_key provided."
            )
            return default_return
        elif not section_id and not rating_key:
            logger.warn(
                u"Tautulli Libraries :: Datatable media info called but no input provided."
            )
            return default_return
        ...
        if library_details['section_id'] == None:
            logger.debug(
                u"Tautulli Libraries :: Library section_id %s not found." %
                section_id)
            return default_return
        ...
        try:
            ...
            logger.warn(
                u"Tautulli Libraries :: Unable to execute database query for get_datatables_media_info2: %s."
                % e)
            return default_return
        ...
        if refresh or not rows:
            ...
            if library_children:
                ...
                logger.warn(
                    u"Tautulli Libraries :: Unable to get a list of library items."
                )
                return default_return
            ...
            if rating_key:
                try:
                    ...
                    logger.debug(
                        u"Tautulli Libraries :: Unable to create cache file for rating_key %s."
                        % rating_key)
            elif section_id:
                try:
                    outFilePath = os.path.join(
                    ...
                    logger.debug(
                        u"Tautulli Libraries :: Unable to create cache file for section_id %s."
                        % section_id)
        ...
        for order in reversed(sort_order):
            ...
            if rating_key and sort_key == 'sort_title':
                results = sorted(
                    ...
                    reverse=reverse)
            elif sort_key in ('file_size', 'bitrate', 'added_at',
    ...
    def get_media_info_file_sizes(self, section_id=None, rating_key=None):
        ...
        if section_id and not str(section_id).isdigit():
            logger.warn(
                u"Tautulli Libraries :: Datatable media info file size called but invalid section_id provided."
            )
            return False
        elif rating_key and not str(rating_key).isdigit():
            logger.warn(
                u"Tautulli Libraries :: Datatable media info file size called but invalid rating_key provided."
            )
            return False
        ...
        if rating_key:
            ...
            logger.debug(
                u"Tautulli Libraries :: Getting file sizes for section_id %s."
                % section_id)
            try:
                inFilePath = os.path.join(plexpy.CONFIG.CACHE_DIR,
                                          'media_info_%s.json' % section_id)
        ...
        for item in rows:
            if item['rating_key'] and not item['file_size']:
                ...
                metadata = pms_connect.get_metadata_children_details(
                    rating_key=item['rating_key'], get_children=True)

                for child_metadata in metadata:
                    ## TODO: Check list of media info items, currently only grabs first item
                    media_info = media_part_info = {}
                    if 'media_info' in child_metadata and len(
                            child_metadata['media_info']) > 0:
                        media_info = child_metadata['media_info'][0]
                        if 'parts' in media_info and len(
                                media_info['parts']) > 0:
                            media_part_info = next(
                                (p for p in media_info['parts']
                                 if p['selected']), media_info['parts'][0])

                    file_size += helpers.cast_to_int(
                        media_part_info.get('file_size', 0))
        ...
        if rating_key:
            try:
                ...
                logger.debug(
                    u"Tautulli Libraries :: Unable to create cache file with file sizes for rating_key %s."
                    % rating_key)
        elif section_id:
            try:
                ...
                logger.debug(
                    u"Tautulli Libraries :: Unable to create cache file with file sizes for section_id %s."
                    % section_id)
        ...
        if rating_key:
            ...
            logger.debug(
                u"Tautulli Libraries :: File sizes updated for section_id %s."
                % section_id)
    ...
    def set_config(self,
        ...
        if section_id:
            ...
            try:
                ...
                logger.warn(
                    u"Tautulli Libraries :: Unable to execute database query for set_config: %s."
                    % e)
    ...
    def get_details(self, section_id=None):
        ...
        def get_library_details(section_id=section_id):
            ...
            try:
                ...
                logger.warn(
                    u"Tautulli Libraries :: Unable to execute database query for get_details: %s."
                    % e)
                result = []
        ...
        if library_details:
            ...
            logger.warn(
                u"Tautulli Libraries :: Unable to retrieve library %s from database. Requesting library list refresh."
                % section_id)
            ...
            if library_details:
                ...
                logger.warn(
                    u"Tautulli Users :: Unable to retrieve library %s from database. Returning 'Local' library."
                    % section_id)
    ...
    def get_watch_time_stats(self, section_id=None, grouping=None):
        ...
        for days in time_queries:
            try:
                ...
                logger.warn(
                    u"Tautulli Libraries :: Unable to execute database query for get_watch_time_stats: %s."
                    % e)
                result = []
    ...
    def get_user_stats(self, section_id=None, grouping=None):
        ...
        try:
            ...
            logger.warn(
                u"Tautulli Libraries :: Unable to execute database query for get_user_stats: %s."
                % e)
            result = []
    ...
    def get_recently_watched(self, section_id=None, limit='10'):
        ...
        try:
            ...
            logger.warn(
                u"Tautulli Libraries :: Unable to execute database query for get_recently_watched: %s."
                % e)
            result = []
    ...
    def get_sections(self):
        ...
        try:
            ...
            logger.warn(
                u"Tautulli Libraries :: Unable to execute database query for get_sections: %s."
                % e)
            return None
    ...
    def delete_all_history(self, section_id=None):
        ...
        try:
            if section_id.isdigit():
                logger.info(
                    u"Tautulli Libraries :: Deleting all history for library id %s from database."
                    % section_id)
            ...
            logger.warn(
                u"Tautulli Libraries :: Unable to execute database query for delete_all_history: %s."
                % e)
    ...
    def delete(self, section_id=None):
        ...
        try:
            if section_id.isdigit():
                ...
                logger.info(
                    u"Tautulli Libraries :: Deleting library with id %s from database."
                    % section_id)
                monitor_db.action(
                    'UPDATE library_sections SET deleted_section = 1 WHERE section_id = ?',
                    [section_id])
                monitor_db.action(
                    'UPDATE library_sections SET keep_history = 0 WHERE section_id = ?',
                    [section_id])
                monitor_db.action(
                    'UPDATE library_sections SET do_notify = 0 WHERE section_id = ?',
                    [section_id])
                monitor_db.action(
                    'UPDATE library_sections SET do_notify_created = 0 WHERE section_id = ?',
                    [section_id])

                library_cards = plexpy.CONFIG.HOME_LIBRARY_CARDS
                if section_id in library_cards:
                    library_cards.remove(section_id)
                    plexpy.CONFIG.__setattr__('HOME_LIBRARY_CARDS',
                                              library_cards)
                    plexpy.CONFIG.write()
            ...
            logger.warn(
                u"Tautulli Libraries :: Unable to execute database query for delete: %s."
                % e)
    ...
    def undelete(self, section_id=None, section_name=None):
        ...
        try:
            if section_id and section_id.isdigit():
                ...
                if result:
                    logger.info(
                        u"Tautulli Libraries :: Re-adding library with id %s to database."
                        % section_id)
                    monitor_db.action(
                ...
                if result:
                    logger.info(
                        u"Tautulli Libraries :: Re-adding library with name %s to database."
                        % section_name)
                    monitor_db.action(
            ...
            logger.warn(
                u"Tautulli Libraries :: Unable to execute database query for undelete: %s."
                % e)
    ...
    def delete_media_info_cache(self, section_id=None):
        ...
        try:
            if section_id.isdigit():
                [
                    os.remove(os.path.join(plexpy.CONFIG.CACHE_DIR, f))
                    for f in os.listdir(plexpy.CONFIG.CACHE_DIR)
                    if f.startswith('media_info_%s' %
                                    section_id) and f.endswith('.json')
                ]

                logger.debug(
                    u"Tautulli Libraries :: Deleted media info table cache for section_id %s."
                    % section_id)
            ...
            logger.warn(
                u"Tautulli Libraries :: Unable to delete media info table cache: %s."
                % e)
    ...
    def delete_duplicate_libraries(self):
        ...
        try:
            logger.debug(
                u"Tautulli Libraries :: Deleting libraries where server_id does not match %s."
                % server_id)
            monitor_db.action(
            ...
            logger.warn(
                u"Tautulli Libraries :: Unable to delete duplicate libraries: %s."
                % e)


def update_libraries_db_notify():
    logger.info(
        u"Tautulli Libraries :: Upgrading library notification toggles...")
    ...
    for section in sections:
        ...
        if (section_details['do_notify'] == 1 and
            (section_details['section_type'] == 'movie'
             and not plexpy.CONFIG.MOVIE_NOTIFY_ENABLE)
                or (section_details['section_type'] == 'show'
                    and not plexpy.CONFIG.TV_NOTIFY_ENABLE)
                or (section_details['section_type'] == 'artist'
                    and not plexpy.CONFIG.MUSIC_NOTIFY_ENABLE)):
            do_notify = 0
        else:
        ...
        if (section_details['keep_history'] == 1 and
            (section_details['section_type'] == 'movie'
             and not plexpy.CONFIG.MOVIE_LOGGING_ENABLE)
                or (section_details['section_type'] == 'show'
                    and not plexpy.CONFIG.TV_LOGGING_ENABLE)
                or (section_details['section_type'] == 'artist'
                    and not plexpy.CONFIG.MUSIC_LOGGING_ENABLE)):
            keep_history = 0
        else:
            keep_history = section_details['keep_history']

        libraries.set_config(
