...
def add_newsletter_each(newsletter_id=None, notify_action=None, **kwargs):
    if not notify_action:
        logger.debug(
            u"Tautulli NewsletterHandler :: Notify called but no action received."
        )
        return
...
def schedule_newsletters(newsletter_id=None):
    ...
    for newsletter in newsletters_list:
        newsletter_job_name = '{} (newsletter_id {})'.format(
            newsletter['agent_label'], newsletter['id'])

        if newsletter['active']:
            schedule_newsletter_job('newsletter-{}'.format(newsletter['id']),
                                    name=newsletter_job_name,
                                    func=add_newsletter_each,
                                    args=[newsletter['id'], 'on_cron'],
...
def schedule_newsletter_job(newsletter_job_id,
                            name='',
                            func=None,
                            remove_job=False,
                            args=None,
    ...
    if NEWSLETTER_SCHED.get_job(newsletter_job_id):
        if remove_job:
            ...
            logger.info(
                u"Tautulli NewsletterHandler :: Removed scheduled newsletter: %s"
                % name)
        else:
            ...
            logger.info(
                u"Tautulli NewsletterHandler :: Re-scheduled newsletter: %s" %
                name)
    elif not remove_job:
        NEWSLETTER_SCHED.add_job(func,
                                 args=args,
                                 id=newsletter_job_id,
                                 trigger=CronTrigger.from_crontab(cron))
        logger.info(u"Tautulli NewsletterHandler :: Scheduled newsletter: %s" %
                    name)


def notify(newsletter_id=None, notify_action=None, **kwargs):
    logger.info(
        u"Tautulli NewsletterHandler :: Preparing newsletter for newsletter_id %s."
        % newsletter_id)
...
def set_notify_state(newsletter, notify_action, subject, body, message,
    ...
    if newsletter and notify_action:
        ...
        keys = {'timestamp': int(time.time()), 'uuid': newsletter_uuid}
        ...
        logger.error(
            u"Tautulli NewsletterHandler :: Unable to set notify state.")
...
def get_last_newsletter_email_msg_id(newsletter_id, notify_action):
    ...
    result = db.select_single(
        'SELECT email_msg_id FROM newsletter_log '
...
def get_newsletter(newsletter_uuid=None, newsletter_id_name=None):
    ...
    if newsletter_uuid:
        result = db.select_single(
            'SELECT start_date, end_date, uuid, filename FROM newsletter_log '
    ...
    if result:
        ...
        newsletter_file = result['filename'] or 'newsletter_%s-%s_%s.html' % (
        ...
        if newsletter_file in os.listdir(newsletter_folder):
            try:
                ...
                logger.error(
                    u"Tautulli NewsletterHandler :: Failed to retrieve newsletter '%s': %s"
                    % (newsletter_uuid, e))
        else:
            logger.warn(
                u"Tautulli NewsletterHandler :: Newsletter file '%s' is missing."
                % newsletter_file)
