...
class DataFactory(object):
    ...
    def get_datatables_history(self,
                               kwargs=None,
                               custom_where=None,
                               grouping=None):
        ...
        columns = [
            'session_history.reference_id', 'session_history.id',
            'MAX(started) AS date', 'MIN(started) AS started',
            ...
            'session_history.user_id', 'session_history.user',
            '(CASE WHEN users.friendly_name IS NULL OR TRIM(users.friendly_name) = "" \
             THEN users.username ELSE users.friendly_name END) AS friendly_name',
            'platform', 'player', 'ip_address', 'session_history.media_type',
            ...
            'GROUP_CONCAT(session_history.id) AS group_ids', 'NULL AS state',
            'NULL AS session_key'
        ]
        ...
        if plexpy.CONFIG.HISTORY_TABLE_ACTIVITY:
            ...
            custom_where_union = [[c[0].split('.')[-1], c[1]]
                                  for c in custom_where]
            group_by_union = ['session_key']

            columns_union = [
                'NULL AS reference_id', 'NULL AS id', 'started AS date',
                'started', 'stopped',
                ...
                'platform', 'player', 'ip_address', 'media_type', 'rating_key',
                'parent_rating_key', 'grandparent_rating_key', 'full_title',
                ...
                'transcode_decision', 'NULL AS group_count',
                'NULL AS group_ids', 'state', 'session_key'
            ]
        ...
        try:
            ...
            logger.warn(
                u"Tautulli DataFactory :: Unable to execute database query for get_history: %s."
                % e)
            return {
                'recordsFiltered': 0,
                'recordsTotal': 0,
        ...
        for item in history:
            ...
            if item['percent_complete'] >= watched_percent[item['media_type']]:
                watched_status = 1
            elif item['percent_complete'] >= watched_percent[
                    item['media_type']] / 2:
                watched_status = 0.5
            else:
                watched_status = 0
        ...
        dict = {
            ...
            'draw': query['draw'],
            'filter_duration': helpers.human_duration(filter_duration,
                                                      sig='dhm'),
        ...
        for stat in stats_cards:
            if stat == 'top_movies':
                ...
                try:
                    ...
                    logger.warn(
                        u"Tautulli DataFactory :: Unable to execute database query for get_home_stats: top_movies: %s."
                        % e)
                    return None
                ...
                for item in result:
                    row = {
                        ...
                        item['content_rating'],
                        'labels':
                        item['labels'].split(';') if item['labels'] else (),
                        'user':
                        '',
                        'friendly_name':
                        '',
                ...
                try:
                    ...
                    logger.warn(
                        u"Tautulli DataFactory :: Unable to execute database query for get_home_stats: popular_movies: %s."
                        % e)
                    return None
                ...
                home_stats.append({
                    ...
                    'Most Popular Movies',
                    'rows':
                    session.mask_session_info(popular_movies)
                })
                ...
                try:
                    ...
                    logger.warn(
                        u"Tautulli DataFactory :: Unable to execute database query for get_home_stats: top_tv: %s."
                        % e)
                    return None
                ...
                try:
                    ...
                    logger.warn(
                        u"Tautulli DataFactory :: Unable to execute database query for get_home_stats: popular_tv: %s."
                        % e)
                    return None
                ...
                try:
                    ...
                    logger.warn(
                        u"Tautulli DataFactory :: Unable to execute database query for get_home_stats: top_music: %s."
                        % e)
                    return None
                ...
                try:
                    ...
                    logger.warn(
                        u"Tautulli DataFactory :: Unable to execute database query for get_home_stats: popular_music: %s."
                        % e)
                    return None
                ...
                home_stats.append({
                    ...
                    'rows':
                    session.mask_session_info(popular_music)
                })
                ...
                try:
                    ...
                    logger.warn(
                        u"Tautulli DataFactory :: Unable to execute database query for get_home_stats: top_users: %s."
                        % e)
                    return None
                ...
                home_stats.append({
                    ...
                    'Most Active Users',
                    'rows':
                    session.mask_session_info(top_users, mask_metadata=False)
                })
                ...
                try:
                    ...
                    logger.warn(
                        u"Tautulli DataFactory :: Unable to execute database query for get_home_stats: top_platforms: %s."
                        % e)
                    return None
                ...
                for item in result:
                    ...
                    platform = common.PLATFORM_NAME_OVERRIDES.get(
                        item['platform'], item['platform'])
                    platform_name = next(
                        (v for k, v in common.PLATFORM_NAMES.iteritems()
                ...
                home_stats.append({
                    ...
                    'rows':
                    session.mask_session_info(top_platform,
                                              mask_metadata=False)
                })
                ...
                try:
                    ...
                    logger.warn(
                        u"Tautulli DataFactory :: Unable to execute database query for get_home_stats: last_watched: %s."
                        % e)
                    return None

                for item in result:
                    if not item['grandparent_thumb'] or item[
                            'grandparent_thumb'] == '':
                        thumb = item['thumb']
                    else:
                        thumb = item['grandparent_thumb']
                ...
                home_stats.append({
                    ...
                    'Recently Watched',
                    'rows':
                    session.mask_session_info(last_watched)
                })
                ...
                def calc_most_concurrent(title, result):
                    ...
                    for item in result:
                        times.append({
                            'time': str(item['started']) + 'B',
                            'count': 1
                        })
                        times.append({
                            'time': str(item['stopped']) + 'A',
                            'count': -1
                        })
                ...
                try:
                    ...
                    result = monitor_db.select(query)
                    if result:
                        most_concurrent.append(
                            calc_most_concurrent(title, result))
                    ...
                    logger.warn(
                        u"Tautulli DataFactory :: Unable to execute database query for get_home_stats: most_concurrent: %s."
                        % e)
                    return None
    ...
    def get_library_stats(self, library_cards=[]):
        ...
        try:
            ...
            logger.warn(
                u"Tautulli DataFactory :: Unable to execute database query for get_library_stats: %s."
                % e)
            return None
        ...
        library_stats = {
            k: list(v)
            for k, v in groupby(library_stats, key=lambda x: x['section_type'])
        }
    ...
    def get_stream_details(self, row_id=None, session_key=None):
        ...
        if session.get_session_user_id():
            user_cond = 'AND %s.user_id = %s ' % (
                table, session.get_session_user_id())
        ...
        for item in result:
            ...
            if not item['stream_container']:
                ...
                item['stream_container'] = item['transcode_container'] or item[
                    'container']
                item['stream_video_decision'] = item['video_decision']
                item['stream_video_codec'] = item[
                    'transcode_video_codec'] or item['video_codec']
                item['stream_video_width'] = item['transcode_width'] or item[
                    'width']
                item['stream_video_height'] = item['transcode_height'] or item[
                    'height']
                item['stream_audio_decision'] = item['audio_decision']
                item['stream_audio_codec'] = item[
                    'transcode_audio_codec'] or item['audio_codec']
                item['stream_audio_channels'] = item[
                    'transcode_audio_channels'] or item['audio_channels']
    ...
    def get_metadata_details(self, rating_key):
        ...
        filtered_metadata_list = session.filter_session_info(
            metadata_list, filter_key='section_id')
    ...
    def get_total_duration(self, custom_where=None):
        ...
        try:
            ...
            logger.warn(
                u"Tautulli DataFactory :: Unable to execute database query for get_total_duration: %s."
                % e)
            return None
    ...
    def get_session_ip(self, session_key=''):
        ...
        if session_key:
            try:
                query = 'SELECT ip_address FROM sessions WHERE session_key = %d %s' % (
                    int(session_key), user_cond)
                result = monitor_db.select(query)
            except Exception as e:
                logger.warn(
                    u"Tautulli DataFactory :: Unable to execute database query for get_session_ip: %s."
                    % e)
                return ip_address
        else:
            return ip_address
    ...
    def get_img_info(self,
        ...
        if service == 'imgur':
            ...
            logger.warn(
                u"Tautulli DataFactory :: Unable to execute database query for get_img_info: "
        ...
        try:
            ...
            logger.warn(
                u"Tautulli DataFactory :: Unable to execute database query for get_img_info: %s."
                % e)
    ...
    def set_img_info(self,
        ...
        if service == 'imgur':
            table = 'imgur_lookup'
            values = {
                'imgur_title': img_title,
                'imgur_url': img_url,
                'delete_hash': delete_hash
            }
        elif service == 'cloudinary':
            table = 'cloudinary_lookup'
            values = {'cloudinary_title': img_title, 'cloudinary_url': img_url}
        else:
            logger.warn(
                u"Tautulli DataFactory :: Unable to execute database query for set_img_info: "
                "service not provided.")
            return
    ...
    def delete_img_info(self, rating_key=None, service='', delete_all=False):
        ...
        if not rating_key and not delete_all:
            logger.error(
                u"Tautulli DataFactory :: Unable to delete hosted images: rating_key not provided."
            )
            return False
        ...
        if service.lower() == 'imgur':
            ...
            for imgur_info in results:
                if imgur_info['delete_hash']:
                    helpers.delete_from_imgur(
                        delete_hash=imgur_info['delete_hash'],
                        img_title=imgur_info['imgur_title'],
                        fallback=imgur_info['fallback'])

            logger.info(
                u"Tautulli DataFactory :: Deleting Imgur info%s from the database."
                % log_msg)
            result = monitor_db.action(
                'DELETE FROM imgur_lookup WHERE img_hash '
                'IN (SELECT img_hash FROM image_hash_lookup %s)' % where, args)
            ...
            if delete_all:
                ...
                for cloudinary_info in results:
                    helpers.delete_from_cloudinary(
                        rating_key=cloudinary_info['rating_key'])

            logger.info(
                u"Tautulli DataFactory :: Deleting Cloudinary info%s from the database."
                % log_msg)
            result = monitor_db.action(
                'DELETE FROM cloudinary_lookup WHERE img_hash '
                'IN (SELECT img_hash FROM image_hash_lookup %s)' % where, args)

        else:
            logger.error(
                u"Tautulli DataFactory :: Unable to delete hosted images: invalid service '%s' provided."
                % service)
    ...
    def get_poster_info(self, rating_key='', metadata=None, service=None):
        ...
        if poster_key:
            ...
            if service:
                ...
                if img_info:
                    poster_info = {
                        'poster_title': img_info[0]['img_title'],
    ...
    def get_lookup_info(self, rating_key='', metadata=None):
        ...
        lookup_info = {'tvmaze_id': '', 'themoviedb_id': ''}

        if lookup_key:
            try:
                query = 'SELECT tvmaze_id FROM tvmaze_lookup ' \
                        'WHERE rating_key = ?'
                tvmaze_info = monitor_db.select_single(query,
                                                       args=[lookup_key])
                if tvmaze_info:
                ...
                query = 'SELECT themoviedb_id FROM themoviedb_lookup ' \
                        'WHERE rating_key = ?'
                themoviedb_info = monitor_db.select_single(query,
                                                           args=[lookup_key])
                if themoviedb_info:
                    lookup_info['themoviedb_id'] = themoviedb_info[
                        'themoviedb_id']
            except Exception as e:
                logger.warn(
                    u"Tautulli DataFactory :: Unable to execute database query for get_lookup_info: %s."
                    % e)
    ...
    def delete_lookup_info(self, rating_key='', title=''):
        ...
        if rating_key:
            logger.info(
                u"Tautulli DataFactory :: Deleting lookup info for '%s' (rating_key %s) from the database."
                % (title, rating_key))
            result_tvmaze = monitor_db.action(
                'DELETE FROM tvmaze_lookup WHERE rating_key = ?', [rating_key])
            result_themoviedb = monitor_db.action(
                'DELETE FROM themoviedb_lookup WHERE rating_key = ?',
                [rating_key])
    ...
    def get_search_query(self, rating_key=''):
        ...
        if rating_key:
            ...
            result = monitor_db.select(
                query=query, args=[rating_key, rating_key, rating_key])
        else:
            result = []
    ...
    def get_rating_keys_list(self, rating_key='', media_type=''):
        ...
        try:
            ...
            logger.warn(
                u"Tautulli DataFactory :: Unable to execute database query for get_rating_keys_list: %s."
                % e)
            return {}
        ...
        grandparents = {}
        result = monitor_db.select(query=query.format(
        ...
        for item in result:
            ...
            for item in result:
                ...
                for item in result:
                    key = item['media_index'] if item['media_index'] else item[
                        'title']
                    children.update({key: {'rating_key': item['rating_key']}})

                key = item[
                    'parent_media_index'] if match_type == 'index' else item[
                        'parent_title']
                parents.update({
                    key: {
    ...
    def delete_session_history_rows(self, row_id=None):
        ...
        if row_id.isdigit():
            logger.info(
                u"Tautulli DataFactory :: Deleting row id %s from the session history database."
                % row_id)
            session_history_del = \
    ...
    def update_metadata(self, old_key_list='', new_key_list='', media_type=''):
            ...
            for k, v in old.iteritems():
                if k in new:
                    ...
                    if 'children' in old[k]:
                        pairs.update(
                            get_pairs(old[k]['children'], new[k]['children']))
        ...
        if mapping:
            logger.info(
                u"Tautulli DataFactory :: Updating metadata in the database.")
            for old_key, new_key in mapping.iteritems():
                metadata = pms_connect.get_metadata_details(new_key)

                if metadata:
                    if metadata['media_type'] == 'show' or metadata[
                            'media_type'] == 'artist':
                        ...
                        monitor_db.action(
                            ...
                            [new_key, old_key])
                    elif metadata['media_type'] == 'season' or metadata[
                            'media_type'] == 'album':
                        ...
                        monitor_db.action(
                            'UPDATE session_history SET rating_key = ? WHERE rating_key = ?',
                            [new_key, old_key])
                        monitor_db.action(
                            'UPDATE session_history_media_info SET rating_key = ? WHERE rating_key = ?',
                            [new_key, old_key])

                        # update session_history_metadata table
                        self.update_metadata_details(old_key, new_key,
                                                     metadata)
    ...
    def update_metadata_details(self,
                                old_rating_key='',
                                new_rating_key='',
                                metadata=None):
        ...
        if metadata:
            ...
            args = [
                metadata['rating_key'], metadata['parent_rating_key'],
                metadata['grandparent_rating_key'], metadata['title'],
                metadata['parent_title'], metadata['grandparent_title'],
                metadata['original_title'], full_title,
                metadata['media_index'], metadata['parent_media_index'],
                metadata['section_id'], metadata['thumb'],
                metadata['parent_thumb'], metadata['grandparent_thumb'],
                metadata['art'], metadata['media_type'], metadata['year'],
                ...
                metadata['tagline'], metadata['rating'], metadata['duration'],
                metadata['guid'], directors, writers, actors, genres,
                metadata['studio'], labels, old_rating_key
            ]
    ...
    def get_notification_log(self, kwargs=None):
        ...
        columns = [
            'notify_log.id', 'notify_log.timestamp', 'notify_log.session_key',
            'notify_log.rating_key', 'notify_log.user_id', 'notify_log.user',
            'notify_log.notifier_id', 'notify_log.agent_id',
            'notify_log.agent_name', 'notify_log.notify_action',
            'notify_log.subject_text', 'notify_log.body_text',
            'notify_log.success'
        ]
        try:
            ...
            logger.warn(
                u"Tautulli DataFactory :: Unable to execute database query for get_notification_log: %s."
                % e)
            return {
                'recordsFiltered': 0,
                'recordsTotal': 0,
    ...
    def delete_notification_log(self):
        ...
        try:
            logger.info(
                u"Tautulli DataFactory :: Clearing notification logs from database."
            )
            ...
            logger.warn(
                u"Tautulli DataFactory :: Unable to execute database query for delete_notification_log: %s."
                % e)
            return False
    ...
    def get_newsletter_log(self, kwargs=None):
        ...
        columns = [
            'newsletter_log.id', 'newsletter_log.timestamp',
            'newsletter_log.newsletter_id', 'newsletter_log.agent_id',
            'newsletter_log.agent_name', 'newsletter_log.notify_action',
            'newsletter_log.subject_text', 'newsletter_log.body_text',
            'newsletter_log.start_date', 'newsletter_log.end_date',
            'newsletter_log.uuid', 'newsletter_log.success'
        ]
        try:
            ...
            logger.warn(
                u"Tautulli DataFactory :: Unable to execute database query for get_newsletter_log: %s."
                % e)
            return {
                'recordsFiltered': 0,
                'recordsTotal': 0,
    ...
    def delete_newsletter_log(self):
        ...
        try:
            logger.info(
                u"Tautulli DataFactory :: Clearing newsletter logs from database."
            )
            ...
            logger.warn(
                u"Tautulli DataFactory :: Unable to execute database query for delete_newsletter_log: %s."
                % e)
            return False
    ...
    def get_user_devices(self, user_id=''):
        ...
        if user_id:
            try:
                ...
                logger.warn(
                    u"Tautulli DataFactory :: Unable to execute database query for get_user_devices: %s."
                    % e)
                return []
        else:
            return []
    ...
    def get_recently_added_item(self, rating_key=''):
        ...
        if rating_key:
            try:
                ...
                logger.warn(
                    u"Tautulli DataFactory :: Unable to execute database query for get_recently_added_item: %s."
                    % e)
                return []
        else:
            return []
    ...
    def set_recently_added_item(self, rating_key=''):
        ...
        try:
            monitor_db.upsert(table_name='recently_added',
                              key_dict=keys,
                              value_dict=values)
        except Exception as e:
            logger.warn(
                u"Tautulli DataFactory :: Unable to execute database query for set_recently_added_item: %s."
                % e)
            return False
