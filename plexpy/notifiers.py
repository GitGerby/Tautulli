...
def available_notification_agents():
    agents = [
        {
            'label': 'Tautulli Remote Android App',
            'name': 'androidapp',
...
def available_notification_actions():
    actions = [{
        ...
        'name': 'on_change',
        'description':
        'Trigger a notification when a stream changes transcode decision.',
        'subject': 'Tautulli ({server_name})',
        'body':
        ...
        'name': 'on_watched',
        'description':
        'Trigger a notification when a video stream reaches the specified watch percentage.',
        'subject': 'Tautulli ({server_name})',
        ...
        'name': 'on_buffer',
        'description':
        'Trigger a notification when a stream exceeds the specified buffer threshold.',
        'subject': 'Tautulli ({server_name})',
        ...
        'name': 'on_newdevice',
        'description':
        'Trigger a notification when a user streams from a new device.',
        'subject': 'Tautulli ({server_name})',
        ...
        'on_created',
        'description':
        'Trigger a notification when a media item is added to the Plex Media Server.',
        'subject':
        'Tautulli ({server_name})',
        'body':
        ...
        'name': 'on_intdown',
        'description':
        'Trigger a notification when the Plex Media Server cannot be reached internally.',
        'subject': 'Tautulli ({server_name})',
        ...
        'name': 'on_intup',
        'description':
        'Trigger a notification when the Plex Media Server can be reached internally after being down.',
        'subject': 'Tautulli ({server_name})',
        ...
        'name': 'on_extdown',
        'description':
        'Trigger a notification when the Plex Media Server cannot be reached externally.',
        'subject': 'Tautulli ({server_name})',
        ...
        'name': 'on_extup',
        'description':
        'Trigger a notification when the Plex Media Server can be reached externally after being down.',
        'subject': 'Tautulli ({server_name})',
        ...
        'name': 'on_pmsupdate',
        'description':
        'Trigger a notification when an update for the Plex Media Server is available.',
        'subject': 'Tautulli ({server_name})',
        'body':
        'An update is available for the Plex Media Server (version {update_version}).',
        'icon': 'fa-refresh',
        'media_types': ('server', )
    }, {
        ...
        'description':
        'Trigger a notification when an update for the Tautulli is available.',
        'subject': 'Tautulli ({server_name})',
        'body':
        'An update is available for Tautulli (version {tautulli_update_version}).',
        'icon': 'fa-refresh',
        'media_types': ('server', )
    }]
...
def get_notify_agents():
    return tuple(a['name'] for a in sorted(available_notification_agents(),
...
def get_notifiers(notifier_id=None, notify_action=None):
    ...
    result = db.select(
        'SELECT id, agent_id, agent_name, agent_label, friendly_name, %s FROM notifiers %s'
        % (', '.join(notify_actions), where),
        args=args)

    for item in result:
        item['active'] = int(
            any([item.pop(k) for k in item.keys() if k in notify_actions]))
...
def delete_notifier(notifier_id=None):
    ...
    if str(notifier_id).isdigit():
        logger.debug(
            u"Tautulli Notifiers :: Deleting notifier_id %s from the database."
            % notifier_id)
...
def get_notifier_config(notifier_id=None, mask_passwords=False):
    if str(notifier_id).isdigit():
        ...
        logger.error(
            u"Tautulli Notifiers :: Unable to retrieve notifier config: invalid notifier_id %s."
            % notifier_id)
        return None
    ...
    try:
        ...
        notifier_agent = get_agent_class(agent_id=result['agent_id'],
                                         config=config)
    except Exception as e:
        logger.error(
            u"Tautulli Notifiers :: Failed to get notifier config options: %s."
            % e)
        return

    if mask_passwords:
        notifier_agent.config = helpers.mask_config_passwords(
            notifier_agent.config)
    ...
    for k in result.keys():
        if k in notify_actions:
            ...
            if subject is None:
                subject = "" if result['agent_name'] in (
                    'scripts', 'webhook') else notify_actions[k]['subject']
            if body is None:
                body = "" if result['agent_name'] in (
                    'scripts', 'webhook') else notify_actions[k]['body']

            notifier_actions[k] = helpers.cast_to_int(result.pop(k))
            notifier_text[k] = {'subject': subject, 'body': body}
    ...
    result['config_options'] = notifier_agent.return_config_options(
...
def add_notifier_config(agent_id=None, **kwargs):
    if str(agent_id).isdigit():
        ...
        logger.error(
            u"Tautulli Notifiers :: Unable to add new notifier: invalid agent_id %s."
            % agent_id)
        return False

    agent = next(
        (a for a in available_notification_agents() if a['id'] == agent_id),
        None)

    if not agent:
        logger.error(
            u"Tautulli Notifiers :: Unable to retrieve new notification agent: invalid agent_id %s."
            % agent_id)
        return False
    ...
    try:
        ...
        logger.info(
            u"Tautulli Notifiers :: Added new notification agent: %s (notifier_id %s)."
            % (agent['label'], notifier_id))
        blacklist_logger()
        return notifier_id
    except Exception as e:
        logger.warn(
            u"Tautulli Notifiers :: Unable to add notification agent: %s." % e)
        return False
...
def set_notifier_config(notifier_id=None, agent_id=None, **kwargs):
    if str(agent_id).isdigit():
        ...
        logger.error(
            u"Tautulli Notifiers :: Unable to set existing notifier: invalid agent_id %s."
            % agent_id)
        return False
    ...
    if not agent:
        logger.error(
            u"Tautulli Notifiers :: Unable to retrieve existing notification agent: invalid agent_id %s."
            % agent_id)
        return False
    ...
    actions = {
        k: helpers.cast_to_int(kwargs.pop(k))
        for k in kwargs.keys() if k in notify_actions
    }
    subject_text = {
        k: kwargs.pop(k)
        for k in kwargs.keys()
        if k.startswith(notify_actions) and k.endswith('_subject')
    }
    body_text = {
        k: kwargs.pop(k)
        for k in kwargs.keys()
        if k.startswith(notify_actions) and k.endswith('_body')
    }
    notifier_config = {
    ...
    values = {
        ...
        agent['label'],
        'friendly_name':
        kwargs.get('friendly_name', ''),
        'notifier_config':
        json.dumps(agent_class.config),
        'custom_conditions':
        kwargs.get('custom_conditions', json.dumps(DEFAULT_CUSTOM_CONDITIONS)),
        'custom_conditions_logic':
        kwargs.get('custom_conditions_logic', ''),
    }
    ...
    try:
        db.upsert(table_name='notifiers', key_dict=keys, value_dict=values)
        logger.info(
            u"Tautulli Notifiers :: Updated notification agent: %s (notifier_id %s)."
            % (agent['label'], notifier_id))
        blacklist_logger()
        ...
        logger.warn(
            u"Tautulli Notifiers :: Unable to update notification agent: %s." %
            e)
        return False


def send_notification(notifier_id=None,
                      subject='',
                      body='',
                      notify_action='',
    ...
    if notifier_config:
        ...
        logger.debug(
            u"Tautulli Notifiers :: Notification requested but no notifier_id received."
        )
...
class PrettyMetadata(object):
    ...
    def get_music_providers():
        return {'': '', 'plexweb': 'Plex Web', 'lastfm': 'Last.fm'}
    ...
    def get_title(self, divider='-'):
        title = ''
        if self.media_type == 'movie':
            title = '%s (%s)' % (self.parameters['title'],
                                 self.parameters['year'])
        elif self.media_type == 'show':
            title = '%s (%s)' % (self.parameters['show_name'],
                                 self.parameters['year'])
        elif self.media_type == 'season':
            title = '%s - Season %s' % (self.parameters['show_name'],
                                        self.parameters['season_num'])
        elif self.media_type == 'episode':
            title = '%s - %s (S%s %s E%s)' % (
                self.parameters['show_name'], self.parameters['episode_name'],
                self.parameters['season_num'], divider,
                self.parameters['episode_num'])
        elif self.media_type == 'artist':
            title = self.parameters['artist_name']
        elif self.media_type == 'album':
            title = '%s - %s' % (self.parameters['artist_name'],
                                 self.parameters['album_name'])
        elif self.media_type == 'track':
            title = '%s - %s' % (self.parameters['track_name'],
                                 self.parameters['track_artist'])
...
class Notifier(object):
    ...
    def notify(self, subject='', body='', action='', **kwargs):
        if self.NAME not in ('Script', 'Webhook'):
            if not subject and self.config.get('incl_subject', True):
                logger.error(
                    u"Tautulli Notifiers :: %s notification subject cannot be blank."
                    % self.NAME)
                return
            elif not body:
                logger.error(
                    u"Tautulli Notifiers :: %s notification body cannot be blank."
                    % self.NAME)
                return

        return self.agent_notify(subject=subject,
                                 body=body,
                                 action=action,
                                 **kwargs)
    ...
    def make_request(self, url, method='POST', **kwargs):
        logger.info(
            u"Tautulli Notifiers :: Sending {name} notification...".format(
                name=self.NAME))
        ...
        if response and not err_msg:
            logger.info(
                u"Tautulli Notifiers :: {name} notification sent.".format(
                    name=self.NAME))
            return True
            ...
            logger.error(
                u"Tautulli Notifiers :: {name} notification failed.{msg}".
                format(msg=verify_msg, name=self.NAME))
            ...
            if req_msg:
                logger.debug(
                    u"Tautulli Notifiers :: Request response: {}".format(
                        req_msg))
...
class ANDROIDAPP(Notifier):
    ...
    _DEFAULT_CONFIG = {'device_id': '', 'priority': 3}

    _ONESIGNAL_APP_ID = '3b4b666a-d557-4b92-acdf-e2c8c4b95357'

    def agent_notify(self,
                     subject='',
                     body='',
                     action='',
                     notification_id=None,
        ...
        device = mobile_app.get_mobile_devices(
            device_id=self.config['device_id'])
        if not device:
            logger.warn(
                u"Tautulli Notifiers :: Unable to send Android app notification: device not registered."
            )
            return
        else:
            device = device[0]
        ...
        if CRYPTODOME:
            ...
            encrypted_data, gcm_tag = cipher.encrypt_and_digest(
                json.dumps(plaintext_data))
            ...
            logger.warn(
                u"Tautulli Notifiers :: PyCryptodome library is missing. "
        ...
        return self.make_request("https://onesignal.com/api/v1/notifications",
                                 headers=headers,
                                 json=payload)
    ...
    def get_devices(self):
        ...
        try:
            ...
            logger.warn(
                u"Tautulli Notifiers :: Unable to retrieve Android app devices list: %s."
                % e)
            return {'': ''}
    ...
    def _return_config_options(self):
        ...
        if not CRYPTODOME:
            config_option.append({
                ...
                'Warning',
                'description':
                '<strong>The PyCryptodome library is missing. '
            ...
            config_option.append({
                ...
                'Note',
                'description':
                'The PyCryptodome library was found. '
        ...
        if not devices:
            ...
            config_option.append({
                ...
                'name':
                'androidapp_device_id',
                'description':
                'Set your Android app device or '
                '<a data-tab-destination="tabs-android_app" data-toggle="tab" data-dismiss="modal" '
...
class BOXCAR(Notifier):
    ...
    def agent_notify(self, subject='', body='', action='', **kwargs):
        ...
        return self.make_request('https://new.boxcar.io/api/notifications',
                                 params=data)
    ...
    def _return_config_options(self):
        config_option = [{
            ...
            'name': 'boxcar_sound',
            'description':
            'Set the notification sound. Leave blank for the default sound.',
            'input_type': 'select',
...
class BROWSER(Notifier):
    ...
    def agent_notify(self, subject='', body='', action='', **kwargs):
        logger.info(u"Tautulli Notifiers :: {name} notification sent.".format(
            name=self.NAME))
        return True
    ...
    def _return_config_options(self):
        config_option = [{
            ...
            'description':
            'Click to allow browser notifications. You must click this button for each browser.',
            'input_type': 'button'
        }, {
            ...
            'description':
            'Set the number of seconds for the notification to remain visible. \
...
class DISCORD(Notifier):
    ...
    def agent_notify(self, subject='', body='', action='', **kwargs):
        ...
        if self.config['incl_card'] and kwargs.get('parameters',
                                                   {}).get('media_type'):
            ...
            if self.config['color']:
                hex_match = re.match(r'^#([0-9a-fA-F]{3}){1,2}$',
                                     self.config['color'])
                if hex_match:
            ...
            if self.config[
                    'incl_description'] or pretty_metadata.media_type in (
                        'artist', 'album', 'track'):
            ...
            if provider_link:
                ...
                fields.append({
                    ...
                    'value':
                    '[%s](%s)' %
                    (provider_name, provider_link.encode('utf-8')),
                    'inline':
                    True
                })
            if self.config['incl_pmslink']:
                fields.append({
                    ...
                    'View Details',
                    'value':
                    '[Plex Web](%s)' % plex_url.encode('utf-8'),
                    'inline':
                    True
                })
            if fields:
        ...
        return self.make_request(self.config['hook'],
                                 params=params,
                                 headers=headers,
                                 json=data)
    ...
    def _return_config_options(self):
        config_option = [{
            ...
            'description':
            'The Discord username which will be used. Leave blank for webhook integration default.',
            'input_type': 'text'
        }, {
            'label': 'Discord Avatar',
            'value': self.config['avatar_url'],
            'description':
            'The image url for the avatar which will be used. Leave blank for webhook integration default.',
            ...
            'description':
            'The hex color value (starting with \'#\') for the border along the left side of the message attachment.',
            'name': 'discord_color',
            'input_type': 'text'
        }, {
            'label': 'TTS',
            ...
            'value':
            self.config['incl_card'],
            'name':
            'discord_incl_card',
            'description':
            'Include an info card with a poster and metadata with the notifications.<br>'
            ...
            'description':
            'Include a plot summary for movies and TV shows on the info card.',
            'input_type': 'checkbox'
        }, {
            ...
            'description':
            'Include a second link to the media in Plex Web on the info card.',
            'input_type': 'checkbox'
        }, {
            ...
            'description':
            'Use a thumbnail instead of a full sized poster on the info card.',
            'input_type': 'checkbox'
        }, {
            'label':
            'Movie Link Source',
            'value':
            self.config['movie_provider'],
            'name':
            'discord_movie_provider',
            'description':
            'Select the source for movie links on the info cards. Leave blank to disable.<br>'
            'Note: 3rd party API lookup may need to be enabled under the notifications settings tab.',
            'input_type':
            'select',
            'select_options':
            PrettyMetadata().get_movie_providers()
        }, {
            'label':
            'TV Show Link Source',
            'value':
            self.config['tv_provider'],
            'name':
            'discord_tv_provider',
            'description':
            'Select the source for tv show links on the info cards. Leave blank to disable.<br>'
            'Note: 3rd party API lookup may need to be enabled under the notifications settings tab.',
            'input_type':
            'select',
            'select_options':
            PrettyMetadata().get_tv_providers()
        }, {
            'label':
            'Music Link Source',
            'value':
            self.config['music_provider'],
            'name':
            'discord_music_provider',
            'description':
            'Select the source for music links on the info cards. Leave blank to disable.',
            'input_type':
            'select',
            'select_options':
            PrettyMetadata().get_music_providers()
        }]
...
class EMAIL(Notifier):
    ...
    def agent_notify(self, subject='', body='', action='', **kwargs):
        if self.config['html_support']:
            ...
            plain.replace_header('Content-Transfer-Encoding',
                                 'quoted-printable')
            plain.set_payload(
                kwargs.get('plaintext', bleach.clean(body, strip=True)),
                'utf-8')
        ...
        msg['From'] = email.utils.formataddr(
            (self.config['from_name'], self.config['from']))
        ...
        try:
            mailserver = smtplib.SMTP(self.config['smtp_server'],
            ...
            if self.config['smtp_user']:
                mailserver.login(str(self.config['smtp_user']),
                                 str(self.config['smtp_password']))

            mailserver.sendmail(self.config['from'], recipients,
                                msg.as_string())
            logger.info(
                u"Tautulli Notifiers :: {name} notification sent.".format(
                    name=self.NAME))
            ...
            logger.error(
                u"Tautulli Notifiers :: {name} notification failed: {e}".
                format(name=self.NAME, e=str(e).decode('utf-8')))
    ...
    def get_user_emails(self):
        emails = {
            u['email']: u['friendly_name']
            for u in users.Users().get_users() if u['email']
        }
        ...
        user_emails_to = [{
            'value': k,
            'text': v
        } for k, v in user_emails_to.iteritems()]
        user_emails_cc = [{
            'value': k,
            'text': v
        } for k, v in user_emails_cc.iteritems()]
        user_emails_bcc = [{
            'value': k,
            'text': v
        } for k, v in user_emails_bcc.iteritems()]
...
class FACEBOOK(Notifier):
    ...
    def _get_credentials(self, code=''):
        logger.info(
            u"Tautulli Notifiers :: Requesting access token from {name}.".
        ...
        try:
            ...
            response = api.get_access_token_from_code(
                code=code,
            ...
            logger.error(
                u"Tautulli Notifiers :: Error requesting {name} access token: {e}"
                .format(name=self.NAME, e=e))
    ...
    def _post_facebook(self, **data):
        if self.config['group_id']:
            api = facebook.GraphAPI(access_token=self.config['access_token'],
                                    version='2.12')

            try:
                api.put_object(parent_object=self.config['group_id'],
                ...
                logger.error(
                    u"Tautulli Notifiers :: Error sending {name} post: {e}".
                    format(name=self.NAME, e=e))
                return False

        else:
            logger.error(
                u"Tautulli Notifiers :: Error sending {name} post: No {name} Group ID provided."
                .format(name=self.NAME))
            return False
    ...
    def _return_config_options(self):
        config_option = [{
            ...
            'description':
            'Fill in this address for the "Valid OAuth redirect URIs" '
            ...
            'description':
            'Request Facebook authorization. (Ensure you allow the browser pop-up).',
            'input_type': 'button'
        }, {
...
class GROUPME(Notifier):
    ...
    def agent_notify(self, subject='', body='', action='', **kwargs):
        ...
        if self.config['incl_poster'] and kwargs.get('parameters'):
            ...
            result = pmsconnect.PmsConnect().get_image(
                img=pretty_metadata.parameters.get('poster_thumb', ''))
            if result and result[0]:
                poster_content = result[0]
            else:
                poster_content = ''
                logger.error(
                    u"Tautulli Notifiers :: Unable to retrieve image for {name}."
                    .format(name=self.NAME))
            ...
            if poster_content:
                ...
                r = requests.post('https://image.groupme.com/pictures',
                                  headers=headers,
                                  data=poster_content)

                if r.status_code == 200:
                    logger.info(
                        u"Tautulli Notifiers :: {name} poster sent.".format(
                            name=self.NAME))
                    r_content = r.json()
                    data['attachments'] = [{
                        'type':
                        'image',
                        'url':
                        r_content['payload']['picture_url']
                    }]
                else:
                    logger.error(
                        u"Tautulli Notifiers :: {name} poster failed: "
                        u"[{r.status_code}] {r.reason}".format(name=self.NAME,
                                                               r=r))
                    logger.debug(
                        u"Tautulli Notifiers :: Request response: {}".format(
                            request.server_message(r, True)))

        return self.make_request('https://api.groupme.com/v3/bots/post',
                                 json=data)

    def _return_config_options(self):
        config_option = [{
            'label': 'GroupMe Access Token',
            'value': self.config['access_token'],
...
class GROWL(Notifier):
    ...
    def agent_notify(self, subject='', body='', action='', **kwargs):
        ...
        growl = gntp.notifier.GrowlNotifier(applicationName='Tautulli',
        ...
        try:
            ...
            logger.error(
                u"Tautulli Notifiers :: {name} notification failed: network error"
                .format(name=self.NAME))
            return False
        except gntp.notifier.errors.AuthError:
            logger.error(
                u"Tautulli Notifiers :: {name} notification failed: authentication error"
                .format(name=self.NAME))
            return False
...
class HIPCHAT(Notifier):
    ...
    def _return_config_options(self):
        config_option = [{
            'label':
            'Hipchat Custom Integrations URL',
            'value':
            self.config['hook'],
            'name':
            'hipchat_hook',
            'description':
            'Your Hipchat BYO integration URL. You can get a key from'
            ' <a href="' +
            helpers.anon_url('https://www.hipchat.com/addons/') +
            ...
            'Hipchat Emoticon',
            'value':
            self.config['emoticon'],
            'name':
            'hipchat_emoticon',
            'description':
            'Include an emoticon tag at the beginning of text notifications (e.g. (taco)). Leave blank for none.'
            ' Use a stock emoticon or create a custom emoticon'
            ' <a href="' +
            helpers.anon_url('https://www.hipchat.com/emoticons/') +
            '" target="_blank">here</a>.',
            'input_type':
            'text'
        }, {
...
class IFTTT(Notifier):
    ...
    def agent_notify(self, subject='', body='', action='', **kwargs):
        ...
        data = {
            'value1': subject.encode('utf-8'),
            'value2': body.encode('utf-8')
        }

        if self.config['value3']:
            pretty_metadata = PrettyMetadata(kwargs['parameters'])
            data['value3'] = pretty_metadata.parameters.get(
                self.config['value3'], '')
    ...
    def _return_config_options(self):
        config_option = [{
            ...
            'ifttt_key',
            'description':
            'Your IFTTT webhook key. You can get a key from'
            ' <a href="' +
            helpers.anon_url('https://ifttt.com/maker_webhooks') +
            '" target="_blank">here</a>.',
            'input_type':
            'text'
        }, {
            ...
            'ifttt_event',
            'description':
            'The IFTTT maker event to fire. You can include'
            ...
            'name': 'ifttt_value3',
            'description':
            'Optional: Select a parameter to send as <span class="inline-pre">value3</span>.',
            'input_type': 'select',
...
class JOIN(Notifier):
    ...
    def agent_notify(self, subject='', body='', action='', **kwargs):
        ...
        if r.status_code == 200:
            ...
            if response_data.get('success'):
                ...
                logger.error(
                    u"Tautulli Notifiers :: {name} notification failed: {msg}".
                    format(name=self.NAME, msg=error_msg))
                return False
        else:
            logger.error(
                u"Tautulli Notifiers :: {name} notification failed: [{r.status_code}] {r.reason}"
                .format(name=self.NAME, r=r))
            logger.debug(u"Tautulli Notifiers :: Request response: {}".format(
                request.server_message(r, True)))
            return False
                ...
                if r.status_code == 200:
                    ...
                    if response_data.get('success'):
                        ...
                        devices.update({
                            ...
                            for d in response_devices
                        })
                    else:
                        error_msg = response_data.get('errorMessage')
                        logger.error(
                            u"Tautulli Notifiers :: Unable to retrieve {name} devices list: {msg}"
                            .format(name=self.NAME, msg=error_msg))

                else:
                    logger.error(
                        u"Tautulli Notifiers :: Unable to retrieve {name} devices list: [{r.status_code}] {r.reason}"
                        .format(name=self.NAME, r=r))
                    logger.debug(
                        u"Tautulli Notifiers :: Request response: {}".format(
                            request.server_message(r, True)))
    ...
    def _return_config_options(self):
        config_option = [{
            ...
            'name': 'join_api_key',
            'description':
            'Your Join API key. Required for group notifications.',
            'input_type': 'text',
            'refresh': True
        }, {
            'label': 'Device Name(s)',
            ...
            'value':
            self.config['incl_poster'],
            'name':
            'join_incl_poster',
            'description':
            'Include a poster with the notifications.<br>'
            'Note: <a data-tab-destination="tabs-notifications" data-dismiss="modal" '
            'data-target="#notify_upload_posters">Image Hosting</a> '
            'must be enabled under the notifications settings tab.',
            'input_type':
            ...
            'join_movie_provider',
            'description':
            'Select the source for movie links in the notification. Leave blank to disable.<br>'
            'Note: 3rd party API lookup may need to be enabled under the notifications settings tab.',
            'input_type':
            'select',
            'select_options':
            ...
            'join_tv_provider',
            'description':
            'Select the source for tv show links in the notification. Leave blank to disable.<br>'
            'Note: 3rd party API lookup may need to be enabled under the notifications settings tab.',
            'input_type':
            'select',
            'select_options':
            ...
            'join_music_provider',
            'description':
            'Select the source for music links in the notification. Leave blank to disable.',
            'input_type':
            'select',
            'select_options':
            PrettyMetadata().get_music_providers()
...
class MQTT(Notifier):
    ...
    def _return_config_options(self):
        config_option = [{
            ...
            'name': 'mqtt_port',
            'description':
            'The network port for connecting to the MQTT broker.',
            'input_type': 'number'
        }, {
            'label': 'Protocol',
            ...
            'name': 'mqtt_qos',
            'description':
            'The quality of service level to use when publishing the notification.',
            'input_type': 'select',
            'select_options': {
                0: 0,
                1: 1,
            ...
            'name': 'mqtt_retain',
            'description':
            'Set the message to be retained on the MQTT broker.',
            'input_type': 'checkbox'
        }, {
            'label': 'Keep-Alive',
            ...
            'description':
            'Maximum period in seconds before timing out the connection with the broker.',
            'input_type': 'number'
        }]
...
class NMA(Notifier):
    ...
    _DEFAULT_CONFIG = {'api_key': '', 'priority': 0}
    ...
    def agent_notify(self, subject='', body='', action='', **kwargs):
        ...
        if response[self.config['api_key']][u'code'] == u'200':
            ...
            logger.error(
                u"Tautulli Notifiers :: {name} notification failed.".format(
                    name=self.NAME))
            return False

    def _return_config_options(self):
        config_option = [{
            'label': 'NotifyMyAndroid API Key',
            'value': self.config['api_key'],
            'name': 'nma_api_key',
            'description':
            'Your NotifyMyAndroid API key. Separate multiple api keys with commas.',
            'input_type': 'text'
        }, {
            'label': 'Priority',
            'value': self.config['priority'],
...
class OSX(Notifier):
    ...
    def _swizzle(self, cls, SEL, func):
        ...
        new_IMP = self.objc.selector(wrapper,
    ...
    def agent_notify(self, subject='', body='', action='', **kwargs):
        ...
        try:
            self._swizzle(self.objc.lookUpClass('NSBundle'),
                          b'bundleIdentifier', self._swizzled_bundleIdentifier)
            ...
            logger.error(u"Tautulli Notifiers :: {name} failed: {e}".format(
                name=self.NAME, e=e))
            return False
    ...
    def _return_config_options(self):
        config_option = [{
            ...
            'Register Notify App',
            'value':
            self.config['notify_app'],
            'name':
            'osx_notify_app',
            'description':
            'Enter the path/application name to be registered with the Notification Center. '
...
class PLEX(Notifier):
    ...
    def agent_notify(self, subject='', body='', action='', **kwargs):
        ...
        for host in hosts:
            logger.info(
                u"Tautulli Notifiers :: Sending notification command to {name} @ {host}"
                .format(name=self.NAME, host=host))
            try:
                ...
                if not request:
                    ...
                    logger.info(
                        u"Tautulli Notifiers :: {name} notification sent.".
                        format(name=self.NAME))
    ...
    def _return_config_options(self):
        config_option = [{
            'label': 'Plex Home Theater Host Address',
            'value': self.config['hosts'],
            'name': 'plex_hosts',
            'description':
            'Host running Plex Home Theater (eg. http://localhost:3005). Separate multiple hosts with commas (,).',
            'input_type': 'text'
        }, {
            ...
            'name': 'plex_username',
            'description':
            'Username of your Plex Home Theater client API (blank for none).',
            'input_type': 'text'
        }, {
            ...
            'name': 'plex_password',
            'description':
            'Password of your Plex Home Theater client API (blank for none).',
            'input_type': 'password'
        }, {
            ...
            'description':
            'The duration (in seconds) for the notification to stay on screen.',
            'input_type': 'number'
        }, {
            ...
            'name': 'plex_image',
            'description':
            'Full path or URL to an image to display with the notification. Leave blank for the default.',
            'input_type': 'text'
        }]
...
class PROWL(Notifier):
    ...
    """
    NAME = 'Prowl'
    _DEFAULT_CONFIG = {'key': '', 'priority': 0}
    ...
    def agent_notify(self, subject='', body='', action='', **kwargs):
        ...
        return self.make_request('https://api.prowlapp.com/publicapi/add',
                                 headers=headers,
                                 data=data)
...
class PUSHALOT(Notifier):
    ...
    def agent_notify(self, subject='', body='', action='', **kwargs):
        data = {
            'AuthorizationToken': self.config['api_key'],
        ...
        return self.make_request('https://pushalot.com/api/sendmessage',
                                 headers=headers,
                                 data=data)

    def _return_config_options(self):
        config_option = [{
            'label': 'Pushalot API Key',
            'value': self.config['api_key'],
...
class PUSHBULLET(Notifier):
    ...
    def agent_notify(self, subject='', body='', action='', **kwargs):
        data = {'type': 'note', 'body': body.encode('utf-8')}
        ...
        if self.config['incl_poster'] and kwargs.get('parameters',
            ...
            if poster_content:
                poster_filename = 'poster_{}.png'.format(
                    pretty_metadata.parameters['rating_key'])
                file_json = {
                    'file_name': poster_filename,
                    'file_type': 'image/png'
                }
                files = {
                    'file': (poster_filename, poster_content, 'image/png')
                }
                ...
                if r.status_code == 204:
                    ...
                    logger.error(
                        u"Tautulli Notifiers :: Unable to upload image to {name}: "
                        u"[{r.status_code}] {r.reason}".format(name=self.NAME,
                                                               r=r))
                    logger.debug(
                        u"Tautulli Notifiers :: Request response: {}".format(
        ...
        return self.make_request('https://api.pushbullet.com/v2/pushes',
                                 headers=headers,
                                 json=data)
    ...
    def get_devices(self):
        ...
        if self.config['api_key']:
            ...
            try:
                r = requests.get('https://api.pushbullet.com/v2/devices',
                                 headers=headers)
                ...
                if r.status_code == 200:
                    ...
                    devices.update({
                        d['iden']: d['nickname']
                        for d in pushbullet_devices if d['active']
                    })
                else:
                    logger.error(
                        u"Tautulli Notifiers :: Unable to retrieve {name} devices list: "
                        u"[{r.status_code}] {r.reason}".format(name=self.NAME,
                                                               r=r))
                    logger.debug(
                        u"Tautulli Notifiers :: Request response: {}".format(
    ...
    def _return_config_options(self):
        config_option = [{
            ...
            'description':
            'Set your Pushbullet device. If set, will override channel tag. '
...
class PUSHOVER(Notifier):
    ...
    def agent_notify(self, subject='', body='', action='', **kwargs):
        ...
        if self.config['incl_url'] and kwargs.get('parameters',
                                                  {}).get('media_type'):
            # Grab formatted metadata
            pretty_metadata = PrettyMetadata(kwargs['parameters'])
        ...
        if self.config['incl_poster'] and kwargs.get('parameters',
            ...
            if poster_content:
                ...
                files = {
                    'attachment':
                    (poster_filename, poster_content, 'image/png')
                }
                headers = {}

        return self.make_request('https://api.pushover.net/1/messages.json',
                                 headers=headers,
                                 data=data,
                                 files=files)
    ...
    def _return_config_options(self):
        config_option = [{
            ...
            'pushover_retry',
            'description':
            'Set the interval in seconds to keep retrying the notification.<br>'
            ...
            'pushover_expire',
            'description':
            'Set the duration in seconds when the notification will stop retrying.<br>'
            'Note: For priority 2 only. Minimum 30 seconds.',
            'input_type':
            'number'
        }, {
            ...
            'description':
            'Style your messages using these HTML tags: b, i, u, a[href], font[color]',
            'input_type': 'checkbox'
        }, {
            ...
            'description':
            'Include a supplementary URL with the notifications.',
            'input_type': 'checkbox'
        }, {
            'label': 'Include Subject Line',
            'value': self.config['incl_subject'],
...
class SCRIPTS(Notifier):
    ...
    """
    NAME = 'Script'
    _DEFAULT_CONFIG = {'script_folder': '', 'script': '', 'timeout': 30}
    ...
    def __init__(self, config=None):
        ...
        self.prefix_overrides = ('python2', 'python3', 'python', 'pythonw',
                                 'php', 'ruby', 'perl')
    ...
    def run_script(self, script, user_id):
        ...
        env.update({
            ...
            '',
            'TAUTULLI_URL':
            helpers.get_plexpy_url(hostname='localhost'),
            'TAUTULLI_PUBLIC_URL':
            plexpy.CONFIG.HTTP_BASE_URL + plexpy.HTTP_ROOT,
            'TAUTULLI_APIKEY':
        ...
        try:
            ...
            try:
                ...
                logger.debug(
                    u"Tautulli Notifiers :: Subprocess returned with status code %s."
                    % status)
            finally:
                if timer:
                    timer.cancel()
    ...
    def kill_script(self, process):
        ...
        logger.warn(
            u"Tautulli Notifiers :: Script exceeded timeout limit of %d seconds. "
    ...
    def agent_notify(self, subject='', body='', action='', **kwargs):
        ...
        logger.debug(
            u"Tautulli Notifiers :: Trying to run notify script, action: %s, arguments: %s"
            % (action, script_args))
        ...
        if action and not script:
            logger.debug(
                u"Tautulli Notifiers :: No script selected for action %s, exiting..."
                % action)
            return
        elif not script:
            logger.debug(
                u"Tautulli Notifiers :: No script selected, exiting...")
            return
        ...
        if script_args:  # and os.name == 'nt':
            script_args = [
                arg.encode(plexpy.SYS_ENCODING, 'ignore')
        ...
        logger.debug(
            u"Tautulli Notifiers :: Executing script in a new thread.")
        thread = threading.Thread(target=self.run_script,
                                  args=(script, user_id)).start()
...
class SLACK(Notifier):
    ...
    def agent_notify(self, subject='', body='', action='', **kwargs):
        ...
        if self.config['incl_card'] and kwargs.get('parameters',
            ...
            attachment = {'fallback': 'Image for %s' % title, 'title': title}

            if self.config['color'] and re.match(r'^#(?:[0-9a-fA-F]{3}){1,2}$',
                                                 self.config['color']):
            ...
            if provider_link:
                ...
                fields.append({
                    ...
                    'View Details',
                    'value':
                    '<%s|%s>' % (provider_link, provider_name),
                    'short':
                    True
                })
            if self.config['incl_pmslink']:
    ...
    def _return_config_options(self):
        config_option = [{
            ...
            'name': 'slack_username',
            'description':
            'The Slack username which will be used. Leave blank for webhook integration default.',
            'input_type': 'text'
        }, {
            'label': 'Slack Icon',
            'value': self.config['icon_emoji'],
            'description':
            'The Slack emoji or image url for the icon which will be used. Leave blank for webhook integration default.',
...
class TELEGRAM(Notifier):
        ...
        if self.config['incl_poster'] and kwargs.get('parameters'):
            ...
            if poster_content:
                poster_filename = 'poster_{}.png'.format(
                    pretty_metadata.parameters['rating_key'])
                files = {
                    'photo': (poster_filename, poster_content, 'image/png')
                }
    ...
    def _return_config_options(self):
        config_option = [{
            ...
            'Contact <a href="' +
            helpers.anon_url('https://telegram.me/BotFather') +
            ...
            'label':
            'Telegram Chat ID, Group ID, or Channel Username',
            'value':
            self.config['chat_id'],
            'name':
            'telegram_chat_id',
            'description':
            'Your Telegram Chat ID, Group ID, or @channelusername. '
            'Contact <a href="' +
            helpers.anon_url('https://telegram.me/myidbot') +
            ...
            'description':
            'Style your messages using these HTML tags: b, i, a[href], code, pre.',
            'input_type': 'checkbox'
        }, {
            ...
            'description':
            'Disables automatic link previews for links in the message',
            'input_type': 'checkbox'
        }]
    ...
    def agent_notify(self, subject='', body='', action='', **kwargs):
        if body:
            try:
                ...
                logger.error(
                    u"Tautulli Notifiers :: Invalid {name} json data: {e}".
                    format(name=self.NAME, e=e))
                return False
...
class XBMC(Notifier):
    ...
    def agent_notify(self, subject='', body='', action='', **kwargs):
        ...
        for host in hosts:
            logger.info(
                u"Tautulli Notifiers :: Sending notification command to XMBC @ "
                + host)
            try:
                version = self._sendjson(
                    host, 'Application.GetProperties',
    ...
    def _return_config_options(self):
        config_option = [{
            'label': 'Kodi Host Address',
            'value': self.config['hosts'],
            'name': 'xbmc_hosts',
            'description':
            'Host running Kodi (e.g. http://localhost:8080). Separate multiple hosts with commas (,).',
            'input_type': 'text'
        }, {
            'label': 'Kodi Username',
            'value': self.config['username'],
            'name': 'xbmc_username',
            'description':
            'Username of your Kodi client API (blank for none).',
            'input_type': 'text'
        }, {
            'label': 'Kodi Password',
            'value': self.config['password'],
            'name': 'xbmc_password',
            'description':
            'Password of your Kodi client API (blank for none).',
            'input_type': 'password'
        }, {
            'label': 'Notification Duration',
            'value': self.config['display_time'],
...
class ZAPIER(Notifier):
    ...
    def _return_config_options(self):
        config_option = [{
            ...
            'description':
            'Click this button when prompted on then "Test Webhooks by Zapier" step.',
            'input_type': 'button'
        }, {
            'label':
            'Movie Link Source',
...
def upgrade_config_to_db():
    logger.info(
        u"Tautulli Notifiers :: Upgrading to new notification system...")
    ...
    for action in get_notify_actions():
        ...
        notify_text[action + '_subject'] = monitoring.get(
            subject_key, defualt_subject_text[action])
        notify_text[action + '_body'] = monitoring.get(
            body_key, defualt_body_text[action])
    ...
    for agent in get_notify_agents():
        ...
        for action in get_notify_actions():
            ...
            agent_actions[action] = helpers.cast_to_int(
                agent_config.get(a_key, 0))
        ...
        if any(agent_actions.values()):
            ...
            for conf, val in notifier_default_config.iteritems():
                c_key = agent_config_key + '_' + config_key_overrides.get(
                    agent, {}).get(conf, conf)
                notifier_config[agent + '_' + conf] = agent_config.get(
                    c_key, val)
            ...
            if agent == 'scripts':
                ...
                for script, actions in script_actions.items():
                    if any(agent_actions[a] for a in actions):
                        ...
                        set_notifier_config(notifier_id=notifier_id,
                                            agent_id=agent_id,
                                            **temp_config)
...
def check_browser_enabled():
    ...
    for n in get_notifiers():
        if n['agent_id'] == 17 and n['active']:
            ...
            BROWSER_NOTIFIERS[
                n['id']] = notifier_config['config']['auto_hide_delay']


def get_browser_notifications():
    db = database.MonitorDatabase()
    result = db.select(
        'SELECT notifier_id, subject_text, body_text FROM notify_log '
