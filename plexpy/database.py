...
def clear_history_tables():
    logger.debug(
        u"Tautulli Database :: Deleting all session_history records... No turning back now bub."
    )
    monitor_db = MonitorDatabase()
...
def make_backup(cleanup=False, scheduler=False):
    ...
    if scheduler:
        backup_file = 'tautulli.backup-%s.sched.db' % arrow.now().format(
            'YYYYMMDDHHmmss')
    else:
        backup_file = 'tautulli.backup-%s.db' % arrow.now().format(
            'YYYYMMDDHHmmss')
            ...
            for file_ in db_files:
                if os.stat(
                    ...
                    try:
                        ...
                        logger.error(
                            u"Tautulli Database :: Failed to delete %s from the backup folder: %s"
                            % (file_, e))

    if backup_file in os.listdir(backup_folder):
        logger.debug(u"Tautulli Database :: Successfully backed up %s to %s" %
                     (db_filename(), backup_file))
        return True
    else:
        logger.error(u"Tautulli Database :: Failed to backup %s to %s" %
                     (db_filename(), backup_file))
        return False
...
class MonitorDatabase(object):
    def __init__(self, filename=FILENAME):
        ...
        self.connection.execute("PRAGMA synchronous = %s" %
                                plexpy.CONFIG.SYNCHRONOUS_MODE)
        # Set database journal mode (default WAL)
        self.connection.execute("PRAGMA journal_mode = %s" %
                                plexpy.CONFIG.JOURNAL_MODE)
        # Set database cache size (default 32MB)
        self.connection.execute("PRAGMA cache_size = -%s" %
                                (get_cache_size() * 1024))
            ...
            while attempts < 5:
                try:
                    ...
                    if "unable to open database file" in e or "database is locked" in e:
                        logger.warn(u"Tautulli Database :: Database Error: %s",
                                    e)
                        attempts += 1
                        time.sleep(1)
                    else:
                        logger.error(
                            u"Tautulli Database :: Database error: %s", e)
                        raise

                except sqlite3.DatabaseError as e:
                    logger.error(
                        u"Tautulli Database :: Fatal Error executing %s :: %s",
                        query, e)
                    raise
    ...
    def upsert(self, table_name, value_dict, key_dict):
        ...
        if self.connection.total_changes == changes_before:
            ...
            try:
                ...
                logger.info(u"Tautulli Database :: Queries failed: %s and %s",
