...
def get_server_resources(return_presence=False, return_server=False, **kwargs):
    ...
    if not server['pms_url_manual'] and server['pms_ssl']:
        ...
        if not server['pms_url']:
            ...
            logger.warn(
                u"Tautulli PlexTV :: Unable to retrieve server URLs. Using user-defined value without SSL."
            )
...
class PlexTV(object):
        ...
        if plextv_response:
            try:
                ...
                if xml_head:
                    user = {
                        'auth_token':
                        xml_head[0].getAttribute('authenticationToken'),
                        'user_id':
                        xml_head[0].getAttribute('id')
                    }
                else:
                    logger.warn(
                        u"Tautulli PlexTV :: Could not get Plex authentication token."
                    )
            except Exception as e:
                logger.warn(
                    u"Tautulli PlexTV :: Unable to parse XML for get_token: %s."
                    % e)
                return None
        ...
        if plextv_response:
            try:
                ...
                if xml_head:
                    ...
                    logger.warn(
                        u"Tautulli PlexTV :: Could not get Plex authentication pin."
                    )
                    return None

            except Exception as e:
                logger.warn(
                    u"Tautulli PlexTV :: Unable to parse XML for get_pin: %s."
                    % e)
                return None
    ...
    def delete_plextv_device(self, device_id='', output_format=''):
        ...
        request = self.request_handler.make_request(
            uri=uri, request_type='DELETE', output_format=output_format)
    ...
    def get_full_users_list(self):
        ...
        shared_servers = self.get_plextv_shared_servers(
            machine_id=plexpy.CONFIG.PMS_IDENTIFIER, output_format='xml')
        ...
        try:
            ...
            logger.warn(
                u"Tautulli PlexTV :: Unable to parse shared server list XML for get_full_users_list: %s."
                % e)
            return []
