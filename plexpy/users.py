...
class Users(object):
    ...
    def get_datatables_unique_ips(self, user_id=None, kwargs=None):
        ...
        columns = [
            'session_history.id', 'MAX(session_history.started) AS last_seen',
    ...
    def set_config(self,
        ...
        if str(user_id).isdigit():
            ...
            try:
                ...
                logger.warn(
                    u"Tautulli Users :: Unable to execute database query for set_config: %s."
                    % e)
    ...
    def get_details(self, user_id=None, user=None, email=None):
        ...
        if user_details:
            ...
            logger.warn(
                u"Tautulli Users :: Unable to retrieve user %s from database. Requesting user list refresh."
            ...
            if user_details:
                ...
                logger.warn(
                    u"Tautulli Users :: Unable to retrieve user %s from database. Returning 'Local' user."
                    % user_id if user_id else user)
    ...
    def get_recently_watched(self, user_id=None, limit='10'):
        ...
        try:
            ...
            logger.warn(
                u"Tautulli Users :: Unable to execute database query for get_recently_watched: %s."
                % e)
            result = []
    ...
    def get_users(self):
        ...
        try:
            ...
            logger.warn(
                u"Tautulli Users :: Unable to execute database query for get_users: %s."
                % e)
            return None
    ...
    def delete_all_history(self, user_id=None):
        ...
        try:
            if str(user_id).isdigit():
                logger.info(
                    u"Tautulli Users :: Deleting all history for user id %s from database."
                    % user_id)
            ...
            logger.warn(
                u"Tautulli Users :: Unable to execute database query for delete_all_history: %s."
                % e)
    ...
    def delete(self, user_id=None):
        ...
        try:
            if str(user_id).isdigit():
                ...
                logger.info(
                    u"Tautulli Users :: Deleting user with id %s from database."
                    % user_id)
                monitor_db.action(
                    'UPDATE users SET deleted_user = 1 WHERE user_id = ?',
                    [user_id])
                monitor_db.action(
                    'UPDATE users SET keep_history = 0 WHERE user_id = ?',
                    [user_id])
                monitor_db.action(
                    'UPDATE users SET do_notify = 0 WHERE user_id = ?',
                    [user_id])
            ...
            logger.warn(
                u"Tautulli Users :: Unable to execute database query for delete: %s."
                % e)
    ...
    def get_tokens(self, user_id=None):
        if user_id:
            try:
                ...
                if result:
                    tokens = {
                        'allow_guest': result['allow_guest'],
    ...
    def get_filters(self, user_id=None):
        ...
        try:
            ...
            logger.warn(
                u"Tautulli Users :: Unable to execute database query for get_filters: %s."
                % e)
            result = {}
    ...
    def set_user_login(self,
        ...
        if user_id is None or str(user_id).isdigit():
            ...
            keys = {'timestamp': int(time.time()), 'user_id': user_id}
            ...
            try:
                ...
                logger.warn(
                    u"Tautulli Users :: Unable to execute database query for set_login_log: %s."
                    % e)
